...
#
#1. Use a single `aws_vpc_security_group_ingress_rule` resource block
#to create security group inbound rules for `app-1-sg` security group. 
#
...
#
#2. Use a single `aws_vpc_security_group_egress_rule` resource block
#to create security group egress rules for `app-2-sg` security group.
#
#* If `description` in CSV is `app-2`, the rule must be associated
# with `app-2-sg` security group.
# Only consider outbound rules, the inbound rules should be ignored.
#
# [!IMPORTANT]  
# Use the `for_each` and `for expression` to iterate over the contents
# of CSV files to fetch necessary data.

resource "aws_security_group" "app-1-sg" {
  name        = "app-${data.aws_subnets.challenge-5-subnets.ids[0]}-sg"
...
resource "aws_security_group" "app-2-sg" {
  name        = "app-${data.aws_subnets.challenge-5-subnets.ids[1]}-sg"
  description = "Allow TLS inbound traffic and all outbound traffic"
  vpc_id      = data.aws_vpc.challenge-5-vpc.id
